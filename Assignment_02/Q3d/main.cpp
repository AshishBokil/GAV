...
GLuint gWorldLocation, lightpos_location, view_location, objColor_location,incr_location,iso_location;
...
static void CreateVertexBuffer()
{
	...
	cout<<endl;	
	ll originSize=(x-1)*(y-1)*(z-1);
	Vector4f *origins=new Vector4f[x*y*z] ;
	unsigned int *origIndices=new unsigned int[originSize];
	ll o=0;
	ll newVIndex = 0;
	for (int k = 0; k < z - 1; k++)
	{
	...
	GL_CALL(glBufferData(GL_ARRAY_BUFFER, originSize * 4 * sizeof(float), origins, GL_STATIC_DRAW));

	GL_CALL(glEnableVertexAttribArray(0));
	GL_CALL(glVertexAttribPointer(0, 4, GL_FLOAT, GL_TRUE, 4 * sizeof(float), 0));
	...
	GL_CALL(glBufferData(GL_ELEMENT_ARRAY_BUFFER, originSize * sizeof(unsigned int), origIndices, GL_STATIC_DRAW));
...
static void onDisplay()
{
	...
	///sending increments
	glUniform3f(incr_location,incr.x,incr.y,incr.z);
	glUniform1f(iso_location,v);
	...
	glDrawElements(GL_POINTS, noOfIndices, GL_UNSIGNED_INT, nullptr);

...
float x=0.0f,y=-1.0f;
int countUp=0,countBelow=0;
GLuint VBO, VAO,VBO2,VAO2;
GLuint gWorldLocation;
int noVertices=0,noPoints=0;
/*model*/
OffModel *model;
/* Constants */
...
static int isCrossing(float y1,float y2,float y3,float yplane){
	if(y1>yplane && y2>yplane && y3>yplane)return 0;
	else if(y1<yplane && y2<yplane && y3<yplane)return 2;
	else return 1;
}
...
//	Vertices[i] = Vector3f(x, y-5.0f, 1.0f);
	...
	countBelow=3*model->numberOfPolygons-1;
	noVertices=3*model->numberOfPolygons;

	for(int i=0;i<model->numberOfPolygons;i++){
		//for(int j=0;j<3;j++){
		float y1=(model->vertices[(model->polygons[i]).v[0]]).y;
		float y2=(model->vertices[(model->polygons[i]).v[1]]).y;
		float y3=(model->vertices[(model->polygons[i]).v[2]]).y;

	//	cout<<y1<<" "<<y2<<" "<<y3<<"\n";
		if(isCrossing(y1,y2,y3,25)==0){
			
		vertices[countUp++]= Vector3f(	(model->vertices[(model->polygons[i]).v[0]]).x,
										y1,
									(model->vertices[(model->polygons[i]).v[0]]).z
									
										);
		vertices[countUp++]= Vector3f(	(model->vertices[(model->polygons[i]).v[1]]).x,
											y2,
									(model->vertices[(model->polygons[i]).v[1]]).z									
										);
		vertices[countUp++]= Vector3f(	(model->vertices[(model->polygons[i]).v[2]]).x,
										y3,
									(model->vertices[(model->polygons[i]).v[2]]).z									
										);
		...
		if(isCrossing(y1,y2,y3,20)==2){
			
		vertices[countBelow--]= Vector3f(	(model->vertices[(model->polygons[i]).v[0]]).x,
										y1,
									(model->vertices[(model->polygons[i]).v[0]]).z
									
										);
		vertices[countBelow--]= Vector3f(	(model->vertices[(model->polygons[i]).v[1]]).x,
											y2,
									(model->vertices[(model->polygons[i]).v[1]]).z									
										);
		vertices[countBelow--]= Vector3f(	(model->vertices[(model->polygons[i]).v[2]]).x,
										y3,
									(model->vertices[(model->polygons[i]).v[2]]).z									
										);
...
//	translateMat.InitTranslationTransform(0.0,translation,0);
//	transform=translateMat*transform;
	
 rotateMat.InitAxisRotateTransform(Vector3f(0,1,0),rotation);
//	rotateMat.InitRotateTransform(0,0,rotation);
	...
	glDrawArrays(GL_TRIANGLES, 0, countUp);
//	GL_CALL(glDrawArrays(GL_POINTS, 0, model));
	GL_CALL(glBindVertexArray(0));

	GL_CALL(glBindVertexArray(VAO));
	translateMat.InitTranslationTransform(0.0,-0.5,0);
	transform=translateMat*transform;
	glUniformMatrix4fv(gWorldLocation, 1, GL_TRUE, &transform.m[0][0]);

	glPolygonMode(GL_FRONT_AND_BACK,GL_LINE);
	glDrawArrays(GL_TRIANGLES, countBelow+1,noVertices-countBelow);
//	GL_CALL(glDrawArrays(GL_POINTS, 0, model));
	GL_CALL(glBindVertexArray(0));
